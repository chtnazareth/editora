...
7.  sensores       → editora-sensor.ts estagio; bloqueante falhou, volta ao 5
...
```bash
# onde estamos
bun {{METODO}}/core/tools/editora-orchestrate.ts status

# a diretiva do próximo estágio (JSON com caminhos já resolvidos)
bun {{METODO}}/core/tools/editora-orchestrate.ts proximo

# abrir o estágio
bun {{METODO}}/core/tools/editora-orchestrate.ts iniciar --estagio <slug> [--unidade cap-07]

# sensores do estágio
bun {{METODO}}/core/tools/editora-sensor.ts estagio --estagio <slug> [--unidade cap-07]

# desfechos
bun {{METODO}}/core/tools/editora-orchestrate.ts reportar --estagio <slug> --resultado aguardando-aprovacao
bun {{METODO}}/core/tools/editora-orchestrate.ts reportar --estagio <slug> --resultado aprovado --entrada "Aprovar"
bun {{METODO}}/core/tools/editora-orchestrate.ts reportar --estagio <slug> --resultado rejeitado --entrada "<retorno>"
bun {{METODO}}/core/tools/editora-orchestrate.ts reportar --estagio <slug> --resultado revisado
```
...
4. **Nunca escreve dentro do molde** (`core/templates/livro/`, embutido no método).
   Ele é somente leitura; todo livro nasce de uma cópia.
...
Reabra o estágio com `editora-state.ts definir-status --estagio <slug> --valor em-andamento`.
