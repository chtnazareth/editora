...
Carregue `editora-compositor-agent` e o conhecimento em `core/knowledge/editora-compositor-agent/`.
...
- Skill de voz própria deste livro, se o autor mantiver uma (`prosa-*` no harness).
  Ausência é o normal em livro novo — a voz nasce em `convencoes-prosa`.
- Estrutura própria que fuja do molde dos 27 capítulos.

Em pasta vazia, tudo isto responde "não existe" — e é o caminho esperado de livro novo.
...
`bun {{METODO}}/core/tools/editora-orchestrate.ts reportar --estagio deteccao-projeto --resultado aguardando-aprovacao`, seguido de `--resultado aprovado`. Sem portão humano nesta fase.
