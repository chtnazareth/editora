 ...
 *
 * Uso: bun core/tools/escritor-doctor.ts [--consertar]
 */
...
import { validarGrafo, type ContextoValidacao } from "./escritor-stage-schema.ts";
import { ANALISADORES } from "./escritor-sensores.ts";
import { compilarGrafo } from "./escritor-graph.ts";
...
function main(): number {
  ...
  const checagens: Checagem[] = [
    checar("estágios presentes", () =>
      estagios.length === 0 ? ["nenhum estágio em core/escritor-common/stages/"] : [],
    ),
    ...
    checar("todo sensor declarado tem analisador", () =>
      sensores.filter((s) => !ANALISADORES[s.id]).map((s) => `sensor "${s.id}" sem função em escritor-sensores.ts`),
    ),
