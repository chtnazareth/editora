#!/usr/bin/env bun
/**
 * editora-sensor.ts — executor dos sensores.
 *
 ...
 * `<obra>/.editora/registro/.sensores/<estagio>/<id>-<correlator>.md`,
 * exatamente como o AI-DLC faz com `.aidlc-sensors/`.
 *
 * Uso:
 *   bun core/tools/editora-sensor.ts rodar --id translates --arquivo cap.md
 *   bun core/tools/editora-sensor.ts estagio --estagio rascunho --unidade cap-01
 *   bun core/tools/editora-sensor.ts listar
 */
...
import { acharArquivoCapitulo, lerEstado } from "./editora-state.ts";
import { novoCorrelator, registrar } from "./editora-audit.ts";
import {
  ANALISADORES,
  lerFrontmatterCampo,
  type Achado,
...
function main(): number {
  switch (process.argv[2]) {
    ...
    default:
      console.error("uso: editora-sensor.ts <rodar|estagio|listar> [opções]");
      return 2;
  }
}
