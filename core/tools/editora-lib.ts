/**
 * editora-lib.ts — núcleo compartilhado do motor Editora.
 *
...
export interface Estagio {
   ...
   * escrito fora de `.editora/`. Análogo de `workspace_requires` no AI-DLC.
   */
...
export function dirEstagios(): string {
  return join(dirCore(), "editora-common", "stages");
}
 ...
 * achar uma pasta `.editora/`. Retorna null se não houver obra ativa.
 */
export function acharObra(inicio: string = process.cwd()): string | null {
  let atual = resolve(inicio);
  for (;;) {
    if (existsSync(join(atual, ".editora"))) return atual;
...
export function carregarGrafo(caminho: string = caminhoGrafo()): Grafo {
  if (!existsSync(caminho)) {
    throw new Error(
      `Grafo não compilado (${caminho}). Rode: bun core/tools/editora-graph.ts compilar`,
    );
  }
...
export function carregarGradeEscopos(
  ...
  if (!existsSync(caminho)) {
    throw new Error(
      `Grade de escopos não compilada (${caminho}). Rode: bun core/tools/editora-graph.ts compilar`,
    );
  }
