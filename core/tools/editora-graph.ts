#!/usr/bin/env bun
/**
 * editora-graph.ts — compila os arquivos de estágio (.md com frontmatter) em
 ...
 *
 * Uso:
 *   bun core/tools/editora-graph.ts compilar [--checar]
 *   bun core/tools/editora-graph.ts mostrar [--fase ideacao] [--escopo romance]
 */
...
function main(): number {
  ...
  switch (comando) {
    ...
    default:
      console.error("uso: editora-graph.ts <compilar [--checar] | mostrar [--fase F] [--escopo E]>");
      return 2;
  }
}
