#!/usr/bin/env bun
/**
 * escritor-harness.ts — gera a superfície do Claude Code a partir do núcleo.
 *
 ...
 * em qualquer persona.
 *
 * Uso: bun core/tools/escritor-harness.ts gerar [--checar]
 */

import { existsSync, mkdirSync, readFileSync, readdirSync, rmSync, writeFileSync } from "node:fs";
import { join } from "node:path";
import { carregarAgentes, carregarGrafo, raizMetodo } from "./escritor-lib.ts";
...
<!--
  GERADO por core/tools/escritor-harness.ts a partir de core/agents/${slug}.md
...
function main(): number {
  ...
  if (comando !== "gerar") {
    console.error("uso: escritor-harness.ts gerar [--checar]");
    return 2;
  }
