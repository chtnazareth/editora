...
`escritor-enredo-agent` lidera. Carregue a skill `metodo-martin` — este estágio é a aplicação direta dela.
...
`escritor-leitor-beta-agent` lê o outline como leitor: a cena promete alguma coisa? Emoji: 🎬. Portão padrão (2 opções — estágio de Construção).
