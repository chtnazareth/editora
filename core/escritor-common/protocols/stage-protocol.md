...
Contrato de comportamento de **todo** estágio do AI-DLC Escritor. Porte de
...
Onde este documento e o motor (`escritor-orchestrate.ts`) divergirem, o motor vence:
...
3. Reportar todo desfecho pelo `escritor-orchestrate.ts`, nunca escrevendo estado à mão.
...
3. `bun {{METODO}}/core/tools/escritor-orchestrate.ts reportar --estagio <slug> --resultado aguardando-aprovacao`
...
Estado vive em `<obra>/.escritor/estado.json`, espelhado em `estado.md`.

**Você nunca escreve nesses arquivos.** Toda mudança passa por
`escritor-orchestrate.ts` ou `escritor-state.ts`. As marcas do espelho:
...
Cada evento vira linha em `<obra>/.escritor/auditoria/<host>-<data>.md`, em markdown
...
1. `<obra>/.escritor/memoria/{autor,oficina,projeto}.md` — o que já é regra
2. `<metodo>/core/knowledge/escritor-shared/` — princípios do método
3. `<metodo>/core/knowledge/<agente>/` — metodologia do agente
4. `<obra>/.escritor/conhecimento/` — conhecimento próprio da obra
...
| **registro** | `<obra>/.escritor/registro/` — onde os artefatos vivem |
...
|---|---|
| só este livro | linha em `<obra>/.escritor/memoria/projeto.md` |
| todos os livros deste autor | linha em `<obra>/.escritor/memoria/autor.md` |
| o modo de trabalhar, não o conteúdo | linha em `<obra>/.escritor/memoria/oficina.md` |
| é verificável por máquina | manifesto novo em `<metodo>/core/sensors/` + analisador em `escritor-sensores.ts`, e o id entra no `sensores:` do estágio |
